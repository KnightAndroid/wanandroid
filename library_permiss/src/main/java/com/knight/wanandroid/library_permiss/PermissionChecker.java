...
public final class PermissionChecker {
     ...
     *
     * 检查 Activity 的状态是否正常
     * @param activity 目标activty
     * @param debugMode 是否是调试模式
     * @return 是否是检查通过
     */
    public static boolean checkActivityStatus(Activity activity,boolean debugMode){
        //检查当前Activity状态是否正常 如果不少则不请求权限
        if (activity == null) {
            if (debugMode) {
                // 这个 Activity 对象必须是 FragmentActivity 的子类，请直接继承AppCompatActivity
        ...
        if (activity.isFinishing()) {
            if (debugMode) {
                //这个 Activity 对象不能是关闭状态,这种情况常出现在执行异步请求后申请权限，请自行在外层判断
     ...
     * @return 是否检查通过
     */
    public static boolean checkPermissionArgument(List<String> requestPermissions,boolean debugMode) {
    ...
    public static void checkStoragePermission(Context context,List<String> requestPermissions,boolean scopedStorage) {
