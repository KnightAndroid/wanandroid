 ...
 * @descript:调整字体大小的View
 */
public class ChangeSizeView extends View {
    ...
    public ChangeSizeView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // initDefault
        defaultLineWidth = ScreenUtils.dp2px(2);
        defaultCircleRadius = ScreenUtils.dp2px(35);
        lineWidth = ScreenUtils.dp2px(1);
        ...
        mTextPaint.setTextSize(ScreenUtils.sp2px(smallSize));
        ...
        mText1Paint.setTextSize(ScreenUtils.sp2px(bigSize));
        ...
        mText2Paint.setTextSize(ScreenUtils.sp2px(standerSize));
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        height = h;
        width = w - ScreenUtils.dp2px(20);
        circleY = height / 2;
        // 横线宽度是总宽度-2个圆的半径
        itemWidth = (width - 2 * circleRadius) / max;
        int startX;
        // 把可点击点保存起来
        for (int i = 0; i <= max; i++) {
            startX = circleRadius + i * itemWidth + ScreenUtils.dp2px(10);
            points.add(new Point(startX, height / 2));
        }
        //初始刻度
